...
def read_crsp_price(
    ...
    def universe_filter(
        ...
        if market == "Numerai":
            numerai_tickers = pd.read_csv(numeraitable).drop_duplicates(
    ...
    for year in range(startyear, endyear + 1):
        ...
        pricedf = read_crsp_price_data(folder, year)

        if pricedf is not None:
            ## Merge with Compustat Link Table
            if os.path.isfile(linktable):
                linktabledf = pd.read_csv(linktable).dropna(
...
### Read Compustat International Price Data
###
###
###
def read_compustat_price(
    sectors,
    inds,
    subinds,
    ...
    def universe_filter(
        ...
        if market == "US":
            exchanges = [11, 12, 14, 15, 16, 17, 18, 21]
            pricedf = pricedf[pricedf["exchange"].isin(exchanges)]
            pricedf = pricedf[pricedf["currency"].isin(["USD"])]
        if market == "GBR":
            exchanges = [194]
            pricedf = pricedf[pricedf["exchange"].isin(exchanges)]
            pricedf = pricedf[pricedf["currency"].isin(["GBP"])]
        if market == "CHE":
            exchanges = [151]
            pricedf = pricedf[pricedf["exchange"].isin(exchanges)]
            pricedf = pricedf[pricedf["currency"].isin(["CHF"])]
        if market == "DEU":
            exchanges = [154, 171]
            pricedf = pricedf[pricedf["exchange"].isin(exchanges)]
            if year >= 1999:
                pricedf = pricedf[pricedf["currency"].isin(["EUR"])]
            else:
                pricedf = pricedf[pricedf["currency"].isin(["DEM"])]
                ## Convert Deutsche Mark to Euro
                ### Set Data Types
                for numerical_col in [
                    "market_cap",
                    "dollar_volume",
                    "adjusted_open",
                    "adjusted_high",
                    ...
                    pricedf[numerical_col] = pricedf[numerical_col].apply(
                        lambda x: float(x) / 1.95583
                    )

        if market == "JPN":
            exchanges = [264, 293]
            pricedf = pricedf[pricedf["exchange"].isin(exchanges)]
            pricedf = pricedf[pricedf["currency"].isin(["JPY"])]
        if market == "AUS":
            exchanges = [106]
            pricedf = pricedf[pricedf["exchange"].isin(exchanges)]
            pricedf = pricedf[pricedf["currency"].isin(["AUD"])]
        if market == "CHN":
            exchanges = [249, 250]
            pricedf = pricedf[pricedf["exchange"].isin(exchanges)]
            pricedf = pricedf[pricedf["currency"].isin(["CNY"])]
        if market == "HKG":
            exchanges = [170]
            pricedf = pricedf[pricedf["exchange"].isin(exchanges)]
            pricedf = pricedf[pricedf["currency"].isin(["HKD"])]
        ## Industry Filter
        pricedf["gsubind"] = pricedf["gsubind"].apply(int)
        pricedf["sector"] = pricedf["gsubind"] // 1000000
        if sectors:
            pricedf = pricedf[pricedf["sector"].isin(sectors)]
        if subinds:
            pricedf = pricedf[pricedf["gsubind"].isin(subinds)]
        if inds:
            pricedf = pricedf[pricedf["gind"].isin(inds)]
        return pricedf
    ...
    for year in range(startyear, endyear + 1):
        ...
        pricedf = read_compustat_price_year(
            folder,
            market,
            year,
        )
        ...
        if pricedf is not None:
            ...
            ## US Data needs to be mapped with name table to get gind and gsubind
            if market == "US":
                usnamedf = pd.read_csv(usnameref)
                pricedf = pricedf.merge(
                    usnamedf[
                        [
                            "gvkey",
            ...
            ### Remove asset without trading volume and price
            pricedf.dropna(
                subset=["gsubind", "market_cap", "dollar_volume", identifier],
                inplace=True,
            )
            ### Universe Filter
            pricedf = universe_filter(
                pricedf,
                market,
                sectors,
                inds,
                subinds,
                year,
            ).drop_duplicates(subset=[identifier, "date"])

            if debug:
                print("Compustat Int Price ", market, year, pricedf.shape)
            ...
            pricedfs.append(pricedf[outputcols])

    if len(pricedfs) > 0:
        return pd.concat(pricedfs, axis=0)
    else:
        return pd.DataFrame()


def read_compustat_international_fundamentals(
    fundamentals_folder="../../data/Compustat_Fundamentals",
    startyear=2016,
    endyear=2021,
):
    ...
    for datayear in range(startyear, endyear + 1, 1):
        fundafile = "{}/international_fundamentals_quarter_{}.parquet".format(
            fundamentals_folder, datayear
        )
        if os.path.isfile(fundafile):
            funda_year = pd.read_parquet(fundafile)
            ...
            funda_year.sort_values(["gvkey", "fdateq"], inplace=True)
            funda_year.drop_duplicates(["gvkey", "fdateq"], inplace=True)
            funda_years.append(funda_year)
